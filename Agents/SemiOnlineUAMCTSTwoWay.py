...
from Networks.ModelNN.StateTransitionModelTwoWay import UncertaintyNN
import config

class SemiOnlineUAMCTSTwoWay(DynaAgent, MCTSAgent):
    name = "SemiOnlineUAMCTSTwoWay"
        ...
        with torch.no_grad():
            ...
            if calculate_uncertainty:
                if not self.use_perfect_uncertainty:
                    ...
                    corrupted_uncertainty = self._uf['network'](one_hot_prev_state, action_index)[0, 0]
                else:
        ...
        if self.backpropagate_idea == 1:
            ...
            while len(selected_node.get_childs()) > 0:
                ...
                for ind, child in enumerate(selected_node.get_childs()):
                    if self.print:
                        print(child.state, child.num_visits, child_values[ind])
