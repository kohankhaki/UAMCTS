...
class MCTSAgentTwoWayOnlineResidualModel(DynaAgent, MCTSAgent):
    name = "MCTSAgentTwoWayOnlineResidualModel"
    ...
    def training_step_transition(self, corrupt_transition_batch):
        ...
        predicted_residual = self._st['network'](prev_state_batch, prev_action_batch)
        total_loss = F.mse_loss(predicted_residual, true_next_states - corrupt_next_states)
    ...
    def model(self, state, action, calculate_uncertainty=False):
        ...
        with torch.no_grad():
            ...
            predicted_residual = self._st['network'](one_hot_prev_state, action_index)
            predicted_next_state = predicted_residual + corrupted_next_state
        is_terminal = False
        reward = 0
